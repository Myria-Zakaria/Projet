  ...
  // Et récupérez dans une variable la valeur passée en paramètre dans l’URL !
      ...
      //echo ("Valeur de id : ");
       //echo ($_GET['id']);
        ...
        echo "<br />";
        echo "  <h2 class=\"heading-separator\"> Détails de l'atelier</h2>";
        echo " </div>";
        
        $req ="SELECT * FROM t_atelier_t
        JOIN t_ressources_t USING(atl_id)
        JOIN t_activite_t USING(pad_id)
        where pad_id ='" .$id_pad. "' and pad_code ='" .$code. "';";
       ...
       // echo ($req);
        ...
        //  echo ($resulat->num_rows);
            echo "<table class=\"table table-bordered\">  <th>Intitule de l'Atelier</th> <th>Détails </th>  <th>Description </th> <th>Date de l'Atelier</th>";
            while($atl = $resulat->fetch_assoc()){
            echo "<tr>";
            echo (' <td> '. $atl['atl_intitule'] . '</td> '.' <td> '.$atl['atl_texte'].' </td> '.' <td> '.$atl['rcs_description'].' </td> '.' <td> '.$atl['atl_date'] . ' </td> ');
            echo "<br />";
            }
        }
        echo "</table>";
